...
export const isMatch = (val: string, pattern: TRegxPattern): boolean =>
  new RegExp(pattern).test(val);

export const maxLength = (val: string, length: number): boolean =>
  !isNull(val) && val.length <= length;
...
export const validatePassword = (val: string): boolean =>
  isMatch(val, regxPattern.password);
export const validateEmail = (val: string): boolean =>
  isMatch(val, regxPattern.email);
